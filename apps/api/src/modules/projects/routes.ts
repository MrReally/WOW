...
export function registerProjectsRoutes(
  ...
  app.get<{ Querystring: { modelId: string; startsAt: string; endsAt: string } }>("/api/reservations/availability", async (req) => {
    const auth = await ctx.auth(req);
    requirePermission(auth, "projects.reservation.manage", "warehouse.issue", "apex.view");
    const q = z.object({ modelId: z.string().uuid(), startsAt: z.string(), endsAt: z.string() }).parse(req.query);
    return service.reservationAvailability(q.modelId, q.startsAt, q.endsAt);
  });

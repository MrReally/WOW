...
const createUserSchema = z.object({
  ...
  telegramUsername: z.string().min(1).nullable().optional(),
...
export function registerPeopleRoutes(
  ...
  app.get<{ Querystring: { status?: People.UserListStatus } }>("/api/people", async (req) => {
    const auth = await ctx.auth(req);
    requirePermission(auth, "people.view", "people.manage", "finance.view", "finance.manage", "operations.finance.view", "operations.finance.manage", "operations.payroll.view", "operations.payroll.manage");
  ...
  app.get<{ Params: { id: string } }>("/api/crew-applications/:id/photo", async (req, reply) => {
    ...
    const photo = await downloadTelegramFile(application.photoFileId);
    if (!photo) throw NotFound("photo", req.params.id);

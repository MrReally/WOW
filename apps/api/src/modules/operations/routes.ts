...
export function registerOperationsRoutes(app:FastifyInstance,ctx:RouteContext,service:Operations.OperationsService){
 ...
 app.post<{Params:{unitId:string}}>("/api/operations/return/:unitId/warehouse",async req=>{const auth=await ctx.auth(req);requirePermission(auth,"warehouse.issue");const body=z.object({projectId:uuid,warehouseId:uuid}).parse(req.body);await service.relocateReturnedUnit(body.projectId,req.params.unitId,body.warehouseId,auth.userId);return{ok:true};});

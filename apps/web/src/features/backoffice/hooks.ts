...
export function useBackofficeCommands() {
  ...
  const createDocument = useMutation({ mutationFn: (input: Operations.OperationDocumentInput) => api.post<Operations.OperationDocumentDTO>("/api/operations/documents", input), onSuccess: () => invalidateBackoffice(qc) });
  const updateDocument = useMutation({ mutationFn: ({id,input}:{id:string;input:Operations.OperationDocumentInput}) => api.patch<Operations.OperationDocumentDTO>(`/api/operations/documents/${id}`, input), onSuccess: () => invalidateBackoffice(qc),meta:{successMessage:"Документ обновлён"} });

...
export function formatCableModel(model: Equipment.EquipmentModelDTO, format: string[] = ["sideA", "arrow", "sideB", "length"]): string {
  ...
  const side = (qty: number, connector: string) => `${qty > 1 ? `${qty}x ` : ""}${connector || "?"}`;
  const parts: Record<string, string> = {
    type: attrs.cableType,
    sideA: side(attrs.sideAQty, attrs.sideAConnector),
    arrow: "->",
    sideB: side(attrs.sideBQty, attrs.sideBConnector),
